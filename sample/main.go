...
func updateCustomer(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "PUT" {
		...
		stmt, err := db.Prepare("UPDATE customers SET CompanyName = ?,ContactTitle = ? , Phone = ? WHERE CustomerID = ?")

		_, err = stmt.Exec(newCompanyName, newContactTitle, newPhone, params["id"])
...
func main() {
	...
	r.HandleFunc("/customers/{id}", delCustomer).Methods("DELETE")
